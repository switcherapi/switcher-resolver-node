...
router.post('/api/criteria', appAuth, clientLimiter, [
...
router.get('/api//criteria/snapshot_check/:version', appAuth, clientLimiter, [
...
router.post('/api//criteria/switchers_check', appAuth, clientLimiter, [
...
router.post('/api//criteria2/switchers_check', appAuth, clientLimiter, [
    check('switchers', 'Switcher Key is required').isArray().isLength({ min: 1 })
], validate, async (req, res) => {
    try {
        const configsFound = await getConfigs({ domain: req.domain, components: req.componentId });
